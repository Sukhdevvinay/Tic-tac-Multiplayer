...
socket.on("PlayerRole",(el)=> {
  console.log("Your Role in this Game : ",el, " with id : ",socket.id);
});
socket.on("alrt",(m)=> {
  alert(m);
})
socket.on("After_click",(d,t)=> {
  let pos = d.id;
  draw(pos);
  // p1 = d["Player1"];
  // p2 = d["Player2"];
...
function clk(position) {
  let h = {
    id:position,
    pl:socket.id
  }
  socket.emit("clicked",h);
}
function draw(position) {
  ...
  if (!a.clicked) {
    ...
    let h1;
    if (i == 0) {
      h1 = `<audio src="/sounds/Ad.mp3" autoplay></audio>`;
      html = `<img src="${"/images/o.png"}" alt="404">`;
      i = 1;
      zero();
      numbers[position] = 0;
      count++;
      ...
      h1 = `<audio src="/sounds/Ad.mp3" autoplay></audio>`;
      html = `<img src="${"/images/Cross.png"}" alt="404">`;
      i = 0;
      cross();
      numbers[position] = 1;
      count++;
    ...
    if (count >= 5 && count < 9) {
      ...
      if (ans == 0) { // Winner 1
        winner_1++;
        html1 = `<audio src="/sounds/winner_sound.mp3" autoplay></audio>`;
